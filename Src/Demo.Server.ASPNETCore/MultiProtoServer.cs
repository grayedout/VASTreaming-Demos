...
namespace VAST.Demo
{
    ...
    class MultiProtoServer : IDisposable
    {
        ...
        public MultiProtoServer()
        {
...
#if NET9_0_OR_GREATER && VAST_FEATURE_WEB_TRANSPORT
            // change to true if you want to run this server
            ...
            // HTTP dispatcher links Kestrel to VASTreaming engine
            this.HttpDispatcher = new VAST.HTTP.HttpServer(VAST.HTTP.HttpServer.Type.AspNetCore, null, 8888, 0);
        ...
        public VAST.HTTP.HttpServer HttpDispatcher { get; private set; }
            ...
            while (this.running)
            {
                ...
                do
                {
                    ...
                    if (videoFileTime <= playbackTime)
                    {
                        ...
                        source.PushMedia(videoStreamIndex, videoBuffer, h264BitstreamPosition, frameSize, timestampBase + videoFileTime, timestampBase + videoFileTime, Common.VersatileBufferFlag.AbsoluteTimestamp);
                    ...
                    if (audioFileTime <= playbackTime)
                    {
                        ...
                        source.PushMedia(audioStreamIndex, audioBuffer, audioBitstreamPosition + 7, frameSize - 7, timestampBase + audioFileTime, timestampBase + audioFileTime, Common.VersatileBufferFlag.AbsoluteTimestamp);
        ...
        private void pushingRoutine2()
        {
                ...
                while (true)
                {
                    ...
                    do
                    {
                        ...
                        if (videoFileTime <= playbackTime)
                        {
                            ...
                            canvas.DrawRect(250, 550 + 12, 780, font.Spacing, paintFill);
                            canvas.DrawText(Program.DisplayText, 250, 550 + font.Spacing + 12, font, paintFont);
                            canvas.Flush();
                            videoSource.SetImage(bmpImage);
            ...
            {
                if (bmpBackground != null) try { bmpBackground.Dispose(); } catch { }
                if (bmpImage != null) try { bmpImage.Dispose(); } catch { }
                if (canvas != null) try { canvas.Dispose(); } catch { }
                if (tf != null) try { tf.Dispose(); } catch { }
                if (font != null) try { font.Dispose(); } catch { }
                if (paintFont != null) try { paintFont.Dispose(); } catch { }
                if (paintFill != null) try { paintFill.Dispose(); } catch { }
            }
